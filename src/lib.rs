...
pub fn download(symbol: &str, start_date: &str, end_date: &str, interval: &str) -> Result<Vec<Frame>, Box<dyn Error>> {
    ...
    let start = Utc.from_utc_datetime(&start_naive.and_hms_opt(0, 0, 0).unwrap()).timestamp();
    let end = Utc.from_utc_datetime(&end_naive.and_hms_opt(0, 0, 0).unwrap()).timestamp();

    let day_range = (start_naive - end_naive).num_days().abs();

    match interval {
        "1m" if day_range > 8 => {
            return Err(format!("Interval '1m' supports max 8 days. You requested: {} days.", day_range).into());
        }
        "2m" | "5m" | "15m" | "30m" | "90m" if day_range > 60 => {
            return Err(format!("Interval '{}' supports max 60 days. You requested: {} days.", interval, day_range).into());
        }
        "60m" if day_range > 730 => {
            return Err(format!("Interval '60m' supports max 730 days. You requested: {} days.", day_range).into());
        }
        _ => {}
    }
